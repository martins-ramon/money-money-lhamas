...
function frame(root, job, total, session) {
  root.innerHTML = '';
  ...
  root.appendChild(el);
  el.querySelector('[data-quit]').onclick = session.cancel;
  el.addEventListener('keydown', e => {
    ...
    const target = arrow ? el.querySelector(`[data-a="${arrow}"]`) : /^[1-9]$/.test(e.key) ? el.querySelectorAll('[data-body] button')[Number(e.key) - 1] : null;
...
export async function playShift(root, job, feedback = () => {}) {
  const session = activity(); session.feedback = feedback;
  const games = { burger, school, cinema, factory, hotel, police };
  try {
    await games[job.id](root, job, session);
...
async function burger(root, job, session) {
  const ui = frame(root, job, 5, session);
  const fillings = ['🥩', '🧀', '🥬', '🍅', '🥓', '🧅'];
  for (let n = 0; n < 5; n++) {
...
async function school(root, job, session) {
  ...
  await ui.until(done => {
    const spawn = () => {
      ...
      s.onclick = () => { s.textContent = '✨'; s.style.background = '#9dd39a'; s.disabled = true; setTimeout(() => s.remove(), 350); if (ui.step()) done(); else spawn(); };
      stage.appendChild(s);
    };
    spawn(); spawn();
  });
...
async function factory(root, job, session) {
  const ui = frame(root, job, 8, session);
  for (let n = 0; n < 8; n++) {
...
async function hotel(root, job, session) {
  const ui = frame(root, job, 6, session);
  const guests = ['🦙', '🐐', '🐑', '🦒', '🐴', '🦌'];
  for (let n = 0; n < 6; n++) {
...
async function police(root, job, session) {
  const ui = frame(root, job, 5, session);
  ...
  for (let n = 0; n < 5; n++) {
    ...
    for (const step of route) { routeEl.textContent += step; const b = buttons.find(x => x.dataset.a === step); b.classList.add('flash'); await ui.wait(450); b.classList.remove('flash'); await ui.wait(150); }
    await ui.wait(400); routeEl.textContent = '❓'.repeat(route.length); buttons.forEach(b => (b.disabled = false));
    let i = 0;
    await ui.until(next => {
...
export function defendMansion(root, seconds = 240, feedback = () => {}) {
  return new Promise(resolve => {
    ...
    let hits = 0, stolen = 0, left = seconds, running = true, paused = document.hidden, streak = 0, bestStreak = 0, lastSwing = -Infinity;
    ...
    const onKey = e => {
      if (e.repeat) return;
      if (e.code === 'Escape') { e.preventDefault(); setPaused(!paused); }
      if (e.code === 'Space' && !paused && !e.target.closest('[data-pause]')) {
        e.preventDefault();
        const cx = stage.clientWidth / 2 - 35, cy = stage.clientHeight / 2 - 42;
        [...thieves].filter(t => t.alive).sort((a, b) => Math.hypot(a.x - cx, a.y - cy) - Math.hypot(b.x - cx, b.y - cy))[0]?.bonk();
      }
    };
    el.addEventListener('keydown', onKey);
    setPaused(paused);
    raf = requestAnimationFrame(loop);
    function finish() { running = false; clearInterval(tick); cancelAnimationFrame(raf); document.removeEventListener('visibilitychange', onVis); el.removeEventListener('keydown', onKey); resolve({ stolen, hits, bestStreak }); }
  });
}
